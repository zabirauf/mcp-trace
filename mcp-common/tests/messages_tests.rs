...
fn test_ipc_message_log_entry_serialization() {
    ...
    match deserialized {
        IpcMessage::LogEntry(entry) => {
            ...
            assert_eq!(format!("{:?}", entry.level), format!("{:?}", log_entry.level));
        }
...
fn test_ipc_message_stats_update_serialization() {
    ...
    match deserialized {
        IpcMessage::StatsUpdate(deserialized_stats) => {
            ...
            assert_eq!(deserialized_stats.successful_requests, stats.successful_requests);
            assert_eq!(deserialized_stats.failed_requests, stats.failed_requests);
            assert_eq!(deserialized_stats.active_connections, stats.active_connections);
            assert_eq!(deserialized_stats.bytes_transferred, stats.bytes_transferred);
        }
...
fn test_ipc_envelope_creation() {
    ...
    let time_diff = (envelope.timestamp - deserialized.timestamp).num_milliseconds().abs();
