...
async fn test_basic_ipc_communication() {
    ...
    let socket_path = temp_dir.path().join("basic.sock").to_string_lossy().to_string();
...
async fn test_multiple_messages_sequence() {
    let temp_dir = tempdir().unwrap();
    let socket_path = temp_dir.path().join("sequence.sock").to_string_lossy().to_string();
...
async fn test_proxy_stats_updates() {
    let temp_dir = tempdir().unwrap();
    let socket_path = temp_dir.path().join("stats.sock").to_string_lossy().to_string();
...
async fn test_concurrent_clients() {
    let temp_dir = tempdir().unwrap();
    let socket_path = temp_dir.path().join("concurrent.sock").to_string_lossy().to_string();
...
async fn test_connection_recovery() {
    let temp_dir = tempdir().unwrap();
    let socket_path = temp_dir.path().join("recovery.sock").to_string_lossy().to_string();
...
async fn test_message_types() {
    let temp_dir = tempdir().unwrap();
    let socket_path = temp_dir.path().join("types.sock").to_string_lossy().to_string();
    ...
    let client_task = tokio::spawn(async move {
        ...
        client.send(IpcMessage::ProxyStarted(proxy_info)).await.unwrap();
        ...
        client.send(IpcMessage::ProxyStopped(proxy_id)).await.unwrap();
    });
