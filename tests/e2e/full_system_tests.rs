...
async fn test_end_to_end_proxy_monitor_communication() {
    ...
    let socket_path = temp_dir.path().join("e2e.sock").to_string_lossy().to_string();
    ...
    for (i, proxy_id) in proxy_ids.iter().enumerate() {
        ...
        proxy_clients[i].send(IpcMessage::ProxyStarted(proxy_info.clone())).await.unwrap();
    }
    ...
    for iteration in 0..5 {
        for (i, proxy_id) in proxy_ids.iter().enumerate() {
            ...
            let request = LogEntry::new(
                LogLevel::Request,
                format!("{{\"id\": \"{}\", \"method\": \"tools/list\", \"params\": {{}}}}", iteration),
                proxy_id.clone(),
            );
            
            let response = LogEntry::new(
                LogLevel::Response,
                format!("{{\"id\": \"{}\", \"result\": {{\"tools\": []}}}}", iteration),
                proxy_id.clone(),
            );
            
            proxy_clients[i].send(IpcMessage::LogEntry(request)).await.unwrap();
            proxy_clients[i].send(IpcMessage::LogEntry(response)).await.unwrap();
            ...
            proxy_clients[i].send(IpcMessage::StatsUpdate(stats)).await.unwrap();
    ...
    proxy_clients[0].send(IpcMessage::ProxyStopped(proxy_ids[0].clone())).await.unwrap();
...
async fn test_error_handling_end_to_end() {
    let temp_dir = tempdir().unwrap();
    let socket_path = temp_dir.path().join("e2e_error.sock").to_string_lossy().to_string();
    ...
    proxy_client.send(IpcMessage::ProxyStarted(proxy_info.clone())).await.unwrap();
    ...
    let error_scenarios = vec![
        (LogLevel::Request, r#"{"id": "1", "method": "invalid_method", "params": {}}"#),
        (LogLevel::Response, r#"{"id": "1", "error": {"code": -32601, "message": "Method not found"}}"#),
        (LogLevel::Error, "Connection timeout to MCP server"),
        (LogLevel::Warning, "Server response took longer than expected"),
        (LogLevel::Request, r#"{"id": "2", "method": "tools/call", "params": {"name": "nonexistent"}}"#),
        (LogLevel::Response, r#"{"id": "2", "error": {"code": -32602, "message": "Invalid params"}}"#),
    ];
    
    for (level, message) in error_scenarios {
        let log_entry = LogEntry::new(level, message.to_string(), proxy_id.clone());
        proxy_client.send(IpcMessage::LogEntry(log_entry)).await.unwrap();
    }
    ...
    proxy_client.send(IpcMessage::StatsUpdate(error_stats.clone())).await.unwrap();
    
    // Process all messages
    for _ in 0..7 { // 6 log entries + 1 stats update
        if let Some(envelope) = connection.receive_message().await.unwrap() {
            let event = match envelope.message {
                IpcMessage::LogEntry(entry) => AppEvent::NewLogEntry(entry),
                IpcMessage::StatsUpdate(stats) => AppEvent::StatsUpdate(stats),
...
async fn test_high_throughput_end_to_end() {
    let temp_dir = tempdir().unwrap();
    let socket_path = temp_dir.path().join("e2e_throughput.sock").to_string_lossy().to_string();
    ...
    let proxy_info = ProxyInfo {
        ...
        listen_address: "127.0.0.1:8080".to_string(),
        target_command: vec!["python".to_string(), "high_throughput_server.py".to_string()],
        status: ProxyStatus::Running,
        stats: ProxyStats::default(),
    };
    ...
    for i in 0..num_requests {
        let request = LogEntry::new(
            LogLevel::Request,
            format!("{{\"id\": \"{}\", \"method\": \"tools/list\", \"params\": {{}}}}", i),
            proxy_id.clone(),
        );
        ...
        if i % 100 == 99 {
            ...
            proxy_client.send(IpcMessage::StatsUpdate(stats)).await.unwrap();
        }
    }
